{{-- resources/views/admin/salones/edit.blade.php --}}
@extends('layouts.app')

@section('title', __('admin.salon_form.title_edit'))

@section('content')
  <div class="px-6 py-4 bg-gray-800 rounded-lg shadow-md">
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-gray-100">
        {{ __('admin.salon_form.title_edit') }}
      </h1>
    ...
    <form action="{{ route('admin.salones.update', $salon) }}"
      ...
      <div>
        ...
        <input id="telefono" name="telefono" type="text"
               value="{{ old('telefono', $salon->telefono) }}"
               class="w-full bg-gray-800 border border-gray-700 text-gray-100 p-2 rounded">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input id="horario_apertura" name="horario_apertura" type="time"
                 value="{{ old('horario_apertura', $salon->horario_apertura) }}"
                 class="w-full bg-gray-800 border border-gray-700 text-gray-100 p-2 rounded"
                 required>
        ...
        <div>
          ...
          <input id="horario_cierre" name="horario_cierre" type="time"
                 value="{{ old('horario_cierre', $salon->horario_cierre) }}"
                 class="w-full bg-gray-800 border border-gray-700 text-gray-100 p-2 rounded"
                 required>
      ...
      <div>
        ...
        <textarea id="descripcion" name="descripcion" rows="4"
                  class="w-full bg-gray-800 border border-gray-700 text-gray-100 p-2 rounded resize-none">
          {{ old('descripcion', $salon->descripcion) }}
        </textarea>
      ...
      <div>
        ...
        <input id="especializacion" name="especializacion" type="text"
               value="{{ old('especializacion', $salon->especializacion) }}"
               class="w-full bg-gray-800 border border-gray-700 text-gray-100 p-2 rounded">
      ...
      <div>
        ...
        <input id="foto" name="foto" type="file"
               class="w-full bg-gray-800 border border-gray-700 text-gray-100 p-2 rounded">
        @if($salon->foto)
          <img src="{{ $salon->foto_url }}" alt="" class="mt-2 w-32 h-32 object-cover rounded">
        @endif
      ...
      <div>
        ...
        <select id="id_usuario" name="id_usuario"
          ...
          @foreach($usuarios as $id => $nombre)
            <option value="{{ $id }}" @selected(old('id_usuario', $salon->id_usuario)==$id)>
              {{ $nombre }}
            </option>
          @endforeach
        </select>
