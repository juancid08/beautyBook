...
@section('title', __('admin.service_form.title_edit'))

@section('content')
  <div class="p-6 bg-gray-800 rounded-lg shadow-md">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold text-white">{{ __('admin.service_form.title_edit') }}</h1>
    ...
    <form action="{{ route('admin.servicios.update', $servicio) }}"
          method="POST"
          class="space-y-6">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <textarea name="descripcion" id="descripcion" rows="4"
                  class="w-full bg-gray-900 text-white border border-gray-700 rounded px-3 py-2">{{ old('descripcion', $servicio->descripcion) }}</textarea>
        @error('descripcion')
          <p class="text-red-400 text-sm mt-1">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div>
        ...
        <select name="id_salon" id="id_salon"
          ...
          @foreach($salones as $id => $nombre)
            <option value="{{ $id }}"
                    @selected(old('id_salon', $servicio->id_salon) == $id)>
              {{ $nombre }}
            </option>
          @endforeach
        </select>
