...
@section('content')
  <div class="px-4 sm:px-6 py-4 bg-gray-800 rounded-lg shadow-md">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 space-y-2 sm:space-y-0">
      <h1 class="text-2xl font-semibold text-gray-100">{{ __('admin.users.header') }}</h1>
    ...
    <div class="overflow-x-auto">
      ...
      <table class="min-w-full divide-y divide-gray-700 hidden md:table">
        <thead class="bg-gray-900">
          <tr>
            <th class="px-4 py-2 text-left text-gray-300 uppercase text-sm">{{ __('admin.users.id') }}</th>
            <th class="px-4 py-2 text-left text-gray-300 uppercase text-sm">{{ __('admin.users.name') }}</th>
            <th class="px-4 py-2 text-left text-gray-300 uppercase text-sm">{{ __('admin.users.email') }}</th>
            <th class="px-4 py-2 text-center text-gray-300 uppercase text-sm">{{ __('admin.users.actions') }}</th>
          </tr>
        </thead>
        <tbody class="bg-gray-800 divide-y divide-gray-700">
          @forelse($usuarios as $u)
            <tr class="@if($loop->even) bg-gray-700 @endif">
              <td class="px-4 py-3 text-gray-200">{{ $u->id_usuario }}</td>
              <td class="px-4 py-3 text-gray-200">{{ $u->nombre }} {{ $u->apellidos }}</td>
              <td class="px-4 py-3 text-gray-200">{{ $u->email }}</td>
      ...
      <div class="space-y-4 md:hidden">
            ...
            <div class="flex justify-between items-start">
              <div class="space-y-1">
                <p class="text-gray-300 text-sm">
                  <span class="font-semibold">{{ __('admin.users.id') }}:</span>
                  {{ $u->id_usuario }}
                </p>
                <p class="text-gray-200 font-medium">{{ $u->nombre }} {{ $u->apellidos }}</p>
        ...
        @empty
          <p class="text-center text-gray-400">{{ __('admin.users.no_records') }}</p>
        @endforelse
      </div>
    </div>
  </div>
